...
Organise your images into two folders:

```
data/
├── real/       ← genuine camera photos (store shelves, products, etc.)
└── screen/     ← photos of screens displaying photos (phone, laptop, tablet)
```

**Critical for `screen/` quality:** the content on the screen should match the `real/` class (shelf photos, product displays). Using random internet images for one class and shelf photos for the other risks training a content classifier, not a forensics classifier.

Recommended minimum: **50 real + 50 screen**. The public datasets below were used for the released model.

### Step 2 — Train

```bash
python train.py --real data/real/ --screen data/screen/
...
```
============================================================
  SalesCode Recapture Detector — Training
============================================================

  Libraries:  pywt=OK  skimage=OK
  Feature dim: 68

[1/5] Loading and extracting features ...
      Augmentation: ON (4× per image)
  [real]   1400 images found in 'data/real'
  [screen] 1813 images found in 'data/screen'

  Total samples : 12852  (5600 real, 7252 screen)
  Extraction    : 312.4s  (~24ms per image)

[3/5] 5-fold stratified cross-validation ...

  ┌─────────────────────────────────────────────┐
  │  CV Accuracy: 0.963 ± 0.008                 │
  │  Per-fold:    0.971 0.958 0.967 0.952 0.967 │
  └─────────────────────────────────────────────┘

[4/5] Training final model on full dataset ...
[5/5] Saving model to 'model.pkl' ...  Saved (3502 KB)
```

> **Important:** The cross-validation accuracy shown is the honest number to report. The training accuracy (also printed) is an optimistic upper bound and should not be cited.
...
| Mobile (on-device, TFLite pipeline) | ~150–200 ms | — | Not yet benchmarked |
| Cloud (AWS Lambda 512MB) | ~100–110 ms | — | Same code, serverless |
...
|---|---|---|
| On-device (phone) | **$0.00** | Runs entirely on the user's device — no network call |
| AWS Lambda (512 MB) | **~$0.0009 / 1,000 images** | ≈ $0.85 / million images |
| | | Lambda free tier covers ~8M images/month |
| Dedicated server (c6i.large) | **~$0.0001 / 1,000 images** | At ~800 img/s with parallelism |

**Recommended deployment:** on-device. The model is 3.5 MB (less than most app icons), runs on CPU only, and eliminates latency, network cost, and the risk of the cheater intercepting a cloud round-trip.
...
│
└── demo/
    ├── app.py          # Flask backend for the live web demo
    └── index.html      # Camera interface (works in any browser)
```
...
Point your camera at a real object → score near 0.  
Display a photo on your laptop and photograph it → score near 1.
...
- **~94 ms per image on a laptop CPU** — fast for server use, borderline for real-time on-device without a TFLite conversion step.
- **Performance degrades on 4K+ OLED screens** at close range and perfect focus without moiré — the highest-end attack case. Color-LBP significantly closes this gap vs. v1 but does not eliminate it.
- **Printouts on glossy paper** are harder than screens — no pixel grid, different specular signature. If this attack vector appears in the wild, a dedicated printout-detection pass (halftone frequency analysis) should be added.
- **The model requires retraining** if the SFA app applies aggressive JPEG compression below quality ~60 before upload — the JPEG artifact features assume reasonable quality (≥70).

---

*Originally developed for a computer vision take-home assessment and later expanded into a standalone image forensics project. Approach, feature engineering, and implementation are original work.*
